...
export function HealthLogSaved({
  ...
  return (
    <section
      className="flex min-h-[calc(100dvh-4rem)] flex-col justify-center text-center"
      aria-live="polite"
    >
      <div className="mx-auto grid size-32 place-items-center rounded-full bg-[#d9f1ef] text-[#58bdb7] shadow-lg shadow-[#58bdb7]/10">
        <span className="grid size-24 place-items-center rounded-full bg-[#68c1bc] text-white">
          <Check aria-hidden="true" className="size-12" strokeWidth={1.8} />
        </span>
      </div>
      ...
      <div className="mt-8 space-y-3">
        <button
          className="min-h-13 w-full rounded-2xl border border-[#e8d0b3] bg-white font-semibold text-[#a96225] active:scale-[0.98]"
          onClick={onEdit}
          type="button"
        >
          Edit this log
        ...
        <button
          className="min-h-14 w-full rounded-2xl bg-[#68c1bc] px-5 font-semibold text-white shadow-lg shadow-[#68c1bc]/20 active:scale-[0.98]"
          onClick={onViewDiary}
          type="button"
        >
          View health diary
