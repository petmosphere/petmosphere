"use client";

import { Check, CircleX } from "lucide-react";
import { useState } from "react";

// 0=Sun … 6=Sat; display Monday-first to match Figma order
const WEEKDAYS = [
  "Sunday",
  "Monday",
  "Tuesday",
  "Wednesday",
  "Thursday",
  "Friday",
  "Saturday",
];
const DISPLAY_ORDER = [1, 2, 3, 4, 5, 6, 0];

interface DaySelectorProps {
  value: number; // 0=Sun … 6=Sat
  onChange: (day: number) => void;
}

export function DaySelector({ onChange, value }: DaySelectorProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [pending, setPending] = useState(value);

  function open() {
    setPending(value);
    setIsOpen(true);
  }

  function close() {
    setIsOpen(false);
  }

  function confirm() {
    onChange(pending);
    setIsOpen(false);
  }

  return (
    <>
      <button
        aria-label={`Day: ${WEEKDAYS[value]}. Tap to change.`}
        className="flex h-11 w-full items-center justify-between rounded-[20px] border-[1.5px] border-[#ed802a] bg-white/60 px-4 transition-opacity active:opacity-70"
        onClick={open}
        type="button"
      >
        <span className="text-[15px] font-semibold text-[#2d2d2d]">
          {WEEKDAYS[value]}
        </span>
        <span
          aria-hidden="true"
          className="text-[14px] font-bold text-[#ed802a]"
        >
          ▾
        </span>
      </button>

      {/* Scrim */}
      <div
        aria-hidden="true"
        className={`fixed inset-0 z-40 bg-black/40 transition-opacity duration-300 ${
          isOpen ? "opacity-100" : "pointer-events-none opacity-0"
        }`}
        onClick={close}
      />

      {/* Bottom sheet — Figma: bg #ffffff, border-radius 24 24 0 0, shadow 0 -4 16 rgba(205,146,85,0.10) */}
      <div
        aria-hidden={!isOpen}
        aria-label="Select day"
        aria-modal="true"
        className={`fixed right-0 bottom-0 left-0 z-50 mx-auto flex max-h-[82dvh] max-w-md flex-col overflow-hidden rounded-t-[24px] bg-white shadow-[0px_-4px_16px_rgba(205,146,85,0.10)] transition-transform duration-300 ease-out ${
          isOpen ? "translate-y-0" : "pointer-events-none translate-y-full"
        }`}
        role="dialog"
      >
        {/* Sheet header */}
        <div className="flex flex-col gap-3 px-6 pt-3 pb-5">
          {/* Drag handle — Figma: 40×4, bg #e0d7cd, border-radius 2 */}
          <div className="mx-auto h-1 w-10 rounded-full bg-[#e0d7cd]" />
          <div className="flex items-center justify-between">
            <span className="text-[18px] leading-[25px] font-bold text-[#2d2d2d]">
              Select Day
            </span>
            <button
              aria-label="Close day selector"
              className="flex size-5 items-center justify-center text-[#7a7a7a] transition-opacity active:opacity-60"
              onClick={close}
              type="button"
            >
              <CircleX aria-hidden="true" className="size-5" />
            </button>
          </div>
        </div>

        {/* Options list — Figma: gap 8px, scrollable */}
        <div className="flex-1 overflow-y-auto px-6">
          <div className="flex flex-col gap-2">
            {DISPLAY_ORDER.map((dayIndex) => {
              const selected = dayIndex === pending;
              return (
                <button
                  aria-pressed={selected}
                  className={`flex h-12 w-full items-center justify-between rounded-xl px-3.5 transition-colors ${
                    selected
                      ? "border-[1.5px] border-[#65bcb5] bg-white/60 shadow-[0px_2px_8px_rgba(101,188,181,0.10)]"
                      : "border border-[#f0e6d8] bg-white/60"
                  }`}
                  key={dayIndex}
                  onClick={() => setPending(dayIndex)}
                  type="button"
                >
                  <span
                    className={`text-[14px] leading-[19px] text-[#2d2d2d] ${
                      selected ? "font-semibold" : "font-medium"
                    }`}
                  >
                    {WEEKDAYS[dayIndex]}
                  </span>
                  {/* Selected: teal circle with white check; unselected: empty circle */}
                  {selected ? (
                    <span
                      aria-hidden="true"
                      className="flex size-5 items-center justify-center rounded-full bg-[#65bcb5]"
                    >
                      <Check className="size-2.5 stroke-[3] text-white" />
                    </span>
                  ) : (
                    <span
                      aria-hidden="true"
                      className="size-[18px] rounded-full border border-[#f0e6d8]"
                    />
                  )}
                </button>
              );
            })}
          </div>
        </div>

        {/* Confirm button — Figma: bg #65bcb5, h 50px, border-radius 24px */}
        <div className="px-6 pt-3 pb-6">
          <button
            className="flex h-[50px] w-full items-center justify-center rounded-[24px] bg-[#65bcb5] text-base font-bold text-white transition-transform duration-150 ease-out active:scale-[0.98]"
            onClick={confirm}
            type="button"
          >
            Confirm
          </button>
        </div>
      </div>
    </>
  );
}
