...
export interface RepeatSelectorOption<T extends string> {
  value: T;
  label: string;
}

export interface RepeatSelectorProps<T extends string> {
  value: T;
  onChange: (value: T) => void;
  options: ReadonlyArray<RepeatSelectorOption<T>>;
  label?: string;
}

export function RepeatSelector<T extends string>({
  label = "Repeat",
  onChange,
  options,
  value,
}: RepeatSelectorProps<T>) {
  ...
  return (
    <>
      ...
      {/* Bottom sheet — Figma: bg #ffffff, border-radius 24 24 0 0, shadow 0 -4 16 rgba(205,146,85,0.10) */}
      <div
        aria-hidden={!isOpen}
        aria-label="Choose repeat cycle"
        aria-modal="true"
      ...
      >
        ...
        <div className="flex flex-col gap-3 px-6 pt-3 pb-5">
          {/* Drag handle — Figma: 40×4, bg #e0d7cd, border-radius 2 */}
          <div className="mx-auto h-1 w-10 rounded-full bg-[#e0d7cd]" />
          {/* Title row — "Repeat Cycle" 18px bold + x-circle close */}
          <div className="flex items-center justify-between">
            <span className="text-[18px] leading-[25px] font-bold text-[#2d2d2d]">
              Repeat Cycle
            </span>
            <button
              aria-label="Close repeat selector"
        ...
        {/* Options list — Figma: gap 8px, scrollable */}
            ...
            {options.map((option) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Selected: filled orange circle with white check; unselected: empty circle */}
                  {selected ? (
                    <span
                      aria-hidden="true"
        ...
        {/* Confirm button — Figma: bg #65bcb5, h 50px, border-radius 24px */}
